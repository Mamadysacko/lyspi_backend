...
export async function sendConfirmationCode(req, res) {
  ...
  const { mail } = req.body;
  if (!mail) return res.status(400).json({ message: 'Email requis' });
  ...
  try {
    ...
    const [rows] = await db.query(
      'SELECT * FROM entreprise WHERE mail = ?',
      [mail]
    );
    ...
    await db.query(
      'UPDATE entreprise SET code_confirmation = ? WHERE mail = ?',
      [code, mail]
    );
...
export async function sendPasswordUpdatedEmail(mail) {
  if (!mail) {
  ...
  try {
    ...
    const [rows] = await db.query(
      'SELECT user FROM entreprise WHERE mail = ?',
      [mail]
    );

    if (rows.length === 0) {
      console.error("Utilisateur introuvable pour l'email :", mail);
      return;
    }
